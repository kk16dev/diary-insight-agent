...
### Phase 3：定期抽出バッチの実装（draft自動生成）

**目的：日記から自動でinsightを抽出してdraft/に保存する**

| タスク                            | 変更対象                          |
| --------------------------------- | --------------------------------- |
| 抽出バッチLambda実装              | `batch/extract_diary/` 新規       |
| EventBridge追加（毎日2:00 JST）   | `infra-cdk/lib/extraction-stack.ts` 新規 |
| 抽出バッチLambdaをCDK追加         | `infra-cdk/lib/extraction-stack.ts` 新規 |
| Secrets ManagerにGitHub Token登録 | AWSコンソール                     |
| バッチの手動実行テスト            | -                                 |

**完了条件：** 毎晩バッチが走り、draft/にデータが蓄積される状態

> **Note:** Phase 3では draft/ に保存するのみ。Phase 4で承認フローを実装し、承認後に references/, ideas/, goals/ に移動する。

### Phase 4：レビュー・承認フローの実装

**目的：draftをレビュー・承認して本番データに移動できるようにする**

| タスク                            | 変更対象                          |
| --------------------------------- | --------------------------------- |
| レビューUI実装                    | `frontend/src/pages/ReviewPage.tsx` 新規 |
| 承認API Lambda実装                | `approval/approval_lambda/` 新規  |
| 承認APIをCDK追加                  | `infra-cdk/lib/approval-stack.ts` 新規 |
| React Router設定                  | `frontend/src/App.tsx` 変更       |

**完了条件：** レビューUIでdraftを承認し、references/, ideas/, goals/ に移動できる状態

> **Note:** レビューUIはチャットUIとは別ページ（`/review`）として実装し、Bounded Contextを分離する。

### Phase 5：仕上げ＆ポートフォリオ化
